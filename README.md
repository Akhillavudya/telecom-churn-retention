...
> git-ignored. A **synthetic 3,000-row sample** ships in `data/sample/` (generated by
> [`src/make_sample_data.py`](src/make_sample_data.py)) so the pipeline runs out of the box; drop the
> real `Client.csv` + `Record.csv` into `data/raw/` to reproduce the headline numbers. See
> [How to run](#how-to-run).

## How to run

Runs from a clean clone out of the box — no data download required:
...
Out of the box this runs on the committed **synthetic sample** (`data/sample/`), so every step executes
and all 11 figures regenerate into `reports/figures/`. The sample is a fake-but-realistic stand-in — its
numbers are **not** the headline results. To reproduce the real figures, drop the real `Client.csv` and
`Record.csv` into `data/raw/` and re-run; the script auto-detects them (it prints `Data source: raw` vs
`sample`). A single seed (`RANDOM_STATE = 42`) makes every run identical.
...
```
├── data/                  # sample/ (synthetic, committed) + raw/ (real, git-ignored) + dictionary
├── src/                   # reproducible pipeline (analysis, sample generator, notebook, slides)
